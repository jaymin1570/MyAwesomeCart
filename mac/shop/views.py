...
def index(request): 
    ...
    # param={'no_of_slides':nSlides,'range':range(nSlides),'product':products}   
...
def search(request):
    ...
    cats= {items['category'] for items in catprods}
    for cat in cats:
        prodtemmp = Product.objects.filter(category=cat)
        prod = [item for item in prodtemmp if searchMatch(query,item)]
        n=len(prod)
        nSlides = (n//4)+ ceil((n/4)-(n//4))
        if n != 0:
            allprods.append([prod,range(1,nSlides),nSlides])
    ...
    if len(allprods) == 0 or len(query)<4:
       params={'msg':"please make sure you enter relavnt query !!"}
